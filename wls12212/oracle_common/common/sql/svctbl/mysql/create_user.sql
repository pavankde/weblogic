...
begin
  ...
  SET @sql = CONCAT('grant select on mysql.tables_priv', ' to ', schema_user, '@''%''');

  call svctbl_execute_sql(@sql);

  SET @sql = CONCAT('grant select on mysql.procs_priv', ' to ', schema_user, '@''%''');

  call svctbl_execute_sql(@sql);

  SET @sql = CONCAT('grant select on mysql.proxies_priv', ' to ', schema_user, '@''%''');
