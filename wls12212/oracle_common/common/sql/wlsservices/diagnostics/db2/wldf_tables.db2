...
BEGIN ATOMIC
  ...
  IF (SELECT COUNT(*) FROM SYSCAT.COLUMNS 
      WHERE TABSCHEMA = schemaName AND TABNAME = 'WLS_HVST' 
      AND COLNAME ='PARTITION_ID') = 0 
  THEN
      SET sqlvar = 'ALTER TABLE WLS_HVST ADD PARTITION_ID VARCHAR(250) DEFAULT NULL';
      call executeWLDFStatement(sqlvar);
    END IF;  
...
BEGIN ATOMIC
  ...
  IF (SELECT COUNT(*) FROM SYSCAT.COLUMNS 
      WHERE TABSCHEMA = schemaName AND TABNAME = 'WLS_HVST' 
      AND COLNAME ='PARTITION_NAME') = 0 
  THEN
      SET sqlvar = 'ALTER TABLE WLS_HVST ADD PARTITION_NAME VARCHAR(250) DEFAULT NULL';
      call executeWLDFStatement(sqlvar);
    END IF;  
...
BEGIN ATOMIC
  ...
  SET sqlvar = 'ALTER TABLE WLS_HVST ALTER COLUMN NAME SET DATA TYPE VARCHAR(500)';
  call executeWLDFStatement(sqlvar);
...
BEGIN ATOMIC
  ...
  IF (SELECT COUNT(*) FROM SYSCAT.COLUMNS 
      ...
      AND COLNAME ='PARTITION_ID') = 0 
  THEN
      SET sqlvar = 'ALTER TABLE WLS_EVENTS ADD PARTITION_ID VARCHAR(250) DEFAULT NULL';
      call executeWLDFStatement(sqlvar);
    END IF;  
...
BEGIN ATOMIC
  ...
  IF (SELECT COUNT(*) FROM SYSCAT.COLUMNS 
      ...
      AND COLNAME ='PARTITION_NAME') = 0 
  THEN
      SET sqlvar = 'ALTER TABLE WLS_EVENTS ADD PARTITION_NAME VARCHAR(250) DEFAULT NULL';
      call executeWLDFStatement(sqlvar);
    END IF;  
