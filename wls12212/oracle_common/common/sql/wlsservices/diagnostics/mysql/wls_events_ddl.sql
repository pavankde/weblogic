...
BEGIN
  ...
  IF NOT EXISTS(
    SELECT * FROM `information_schema`.`COLUMNS`
      WHERE COLUMN_NAME='PARTITION_ID' AND TABLE_NAME='WLS_EVENTS') THEN 
      ALTER TABLE `WLS_EVENTS` ADD `PARTITION_ID` varchar(250) default NULL;
  END IF;

  -- Check for PARTITION_NAME column and add if necessary (12.2.1)
  IF NOT EXISTS(
    SELECT * FROM `information_schema`.`COLUMNS`
      WHERE COLUMN_NAME='PARTITION_NAME' AND TABLE_NAME='WLS_EVENTS') THEN 
      ALTER TABLE `WLS_EVENTS` ADD `PARTITION_NAME` varchar(250) default NULL;
  END IF;
