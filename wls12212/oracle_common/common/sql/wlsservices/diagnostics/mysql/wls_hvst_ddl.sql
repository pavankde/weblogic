DROP PROCEDURE if exists create_alter_wls_hvst
/

CREATE PROCEDURE create_alter_wls_hvst() 
language sql
BEGIN
  CREATE TABLE IF NOT EXISTS WLS_HVST
  (
    RECORDID BIGINT AUTO_INCREMENT PRIMARY KEY,
    TIMESTAMP BIGINT NOT NULL,
    DOMAIN VARCHAR(250) default NULL,
    SERVER VARCHAR(250) default NULL,
    TYPE VARCHAR(250) default NULL,
    NAME VARCHAR(500) default NULL,
    ATTRNAME VARCHAR(250) default NULL,
    ATTRTYPE INT default NULL,
    ATTRVALUE VARCHAR(4000) default NULL,
    WLDFMODULE VARCHAR(250) default NULL,
    PARTITION_ID VARCHAR(250) default NULL,
    PARTITION_NAME VARCHAR(250) default NULL,
    INDEX(TIMESTAMP)
  );

  -- Check for WLDFMODULE column and add if necessary (12.1.2)
  IF NOT EXISTS(
    SELECT * FROM `information_schema`.`COLUMNS`
      WHERE COLUMN_NAME='WLDFMODULE' AND TABLE_NAME='WLS_HVST') THEN 
      ALTER TABLE `WLS_HVST` ADD `WLDFMODULE` VARCHAR(250) default NULL;
  END IF;

  -- Check for PARTITION_ID column and add if necessary (12.2.1)
  IF NOT EXISTS(
    SELECT * FROM `information_schema`.`COLUMNS`
      WHERE COLUMN_NAME='PARTITION_ID' AND TABLE_NAME='WLS_HVST') THEN 
      ALTER TABLE `WLS_HVST` ADD `PARTITION_ID` VARCHAR(250) default NULL;
  END IF;

  -- Check for PARTITION_NAME column and add if necessary (12.2.1)
  IF NOT EXISTS(
    SELECT * FROM `information_schema`.`COLUMNS`
      WHERE COLUMN_NAME='PARTITION_NAME' AND TABLE_NAME='WLS_HVST') THEN 
      ALTER TABLE `WLS_HVST` ADD `PARTITION_NAME` VARCHAR(250) default NULL;
  END IF;

  ALTER TABLE `WLS_HVST` MODIFY `NAME` VARCHAR(500) default NULL;
  
  -- drop unnecessary SCOPE column; likely a C&P 
  -- error from 12.1.3 and earlier scripts
  IF EXISTS(
    SELECT * FROM `information_schema`.`COLUMNS`
      WHERE COLUMN_NAME='SCOPE' AND TABLE_NAME='WLS_HVST') THEN 
      ALTER TABLE `WLS_HVST` DROP COLUMN `SCOPE`;
  END IF;

END
/

CALL create_alter_wls_hvst()
/

DROP PROCEDURE if exists create_alter_wls_hvst
/

