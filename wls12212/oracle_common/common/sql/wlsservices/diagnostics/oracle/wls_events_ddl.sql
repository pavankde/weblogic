...
BEGIN
  ...
  INTO vCtr
  FROM user_tab_columns
  WHERE table_name = 'WLS_EVENTS' AND column_name = 'PARTITION_ID';
  IF vCtr = 0 THEN
    dbms_output.put_line('Creating PARTITION_ID column in WLS_EVENTS table');
    vSQL := 'ALTER TABLE WLS_EVENTS ADD("PARTITION_ID" VARCHAR2(250 BYTE) DEFAULT NULL)';
    EXECUTE IMMEDIATE vSQL;  
  END IF;
  ...
  INTO vCtr
  FROM user_tab_columns
  WHERE table_name = 'WLS_EVENTS' AND column_name = 'PARTITION_NAME';
  IF vCtr = 0 THEN
    dbms_output.put_line('Creating PARTITION_NAME column in WLS_EVENTS table');
    vSQL := 'ALTER TABLE WLS_EVENTS ADD("PARTITION_NAME" VARCHAR2(250 BYTE) DEFAULT NULL)';
    EXECUTE IMMEDIATE vSQL;  
  END IF;
