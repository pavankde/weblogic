...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT COLUMN_NAME  
    FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE COLUMN_NAME = 'PARTITION_ID' AND TABLE_NAME = 'WLS_EVENTS' AND TABLE_SCHEMA = @this_schema) 
  BEGIN
    PRINT 'ADDING COLUMN PARTITION_ID TO WLS_EVENTS'
    ALTER TABLE WLS_EVENTS ADD PARTITION_ID VARCHAR(250) NULL
  END;
  ...
  IF NOT EXISTS (
    SELECT COLUMN_NAME  
    FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE COLUMN_NAME = 'PARTITION_NAME' AND TABLE_NAME = 'WLS_EVENTS' AND TABLE_SCHEMA = @this_schema) 
  BEGIN
    PRINT 'ADDING COLUMN PARTITION_NAME TO WLS_EVENTS'
    ALTER TABLE WLS_EVENTS ADD PARTITION_NAME VARCHAR(250) NULL
  END;
