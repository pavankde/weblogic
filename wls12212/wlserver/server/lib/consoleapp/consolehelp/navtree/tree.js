...
function addNode(parentNode, recursedNodes)
{
	for (var n=0; n<parentNode.children.length; n++)
	{
		...
		if (hcn)
		{
			...
			if (ls)
			{
				...
				if (ino) {
				   ...
				   titleText = titleText + " " + messageCatalog["tree.popup.collapsenode"];
				} else {
	               document.write(tree_image_base + 'plus');
	               titleText = titleText + " " + messageCatalog["tree.popup.expandnode"];
				}
		...
		var altInfo = node.nodeName + ", " + messageCatalog["tree.popup.level"] + " " + recursedNodes.length + ", ";
		if(hcn)
		{
		   if(ino)
		     altInfo += messageCatalog["tree.popup.expanded"] + ", ";
		   else
		     altInfo += messageCatalog["tree.popup.collapsed"] + ", ";
		}
		altInfo +=  + (n+1) + " " + messageCatalog["tree.popup.of"] + " " + parentNode.children.length;
		
		if(ish)
		  altInfo += ", " + messageCatalog["tree.popup.selected"];
