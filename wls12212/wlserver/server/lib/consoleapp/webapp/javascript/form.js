...
function Dependency_doDependency (selectedValue, control) {
  ...
  if (!donotProcess) {
    ...
    if (depControl !== null) {
      ...
      // check if the dependent control is a button. In case of buttons we need to set the correct class in addition
      // to the disabled attribute
      if(depControl.type && (depControl.type === "button")) {
        if(enabled) {
          enableButton(depControl);
        } else {
          disableButton(depControl);
        }
      }
