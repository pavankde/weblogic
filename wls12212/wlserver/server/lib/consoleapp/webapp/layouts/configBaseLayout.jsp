...
<!--
  ...
  Use when you want:
  Fixed: save button or need lock message when not read only with /save action in jpf
    ...
    <c:if test="${not formInstance.readOnly}">
        ...
        <c:when test="${chgctr.lockOwner &amp;&amp; not chgctr.activateInProgress}">
            <div class="upperButtonBar">
              <wl:button-bar>
                 <wl:button-bar-button labelid="button.save" styleClass="buttons" action="/save"/>
              </wl:button-bar>
            </div>
        </c:when>
        <c:otherwise>
            ...
            <div class="upperButtonBar">
              <wl:button-bar>
                 <wl:button-bar-button labelid="button.save" readOnly="true"/>
              </wl:button-bar>
            </div>
        </c:otherwise>
        </c:choose>
